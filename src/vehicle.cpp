    ...
    */
    ...
    if (get_vehicle_ahead(predictions, lane, vehicle_ahead)) {
        ...
        if (get_vehicle_behind(predictions, lane, vehicle_behind)) {
            ...
            double max_velocity_in_front = (vehicle_ahead.s - this->s - this->preferred_buffer)/this->dt + vehicle_ahead.v - 0.5 * (this->a) * this->dt;
    ...
    new_accel = (new_velocity - this->v)/this->dt; //Equation: (v_1 - v_0)/t = acceleration
    new_position = this->s + new_velocity * this->dt + new_accel / 2.0 * pow(this->dt,2);
...
vector<Vehicle> Vehicle::prep_lane_change_trajectory(string state, map<int, vector<Vehicle>> predictions) {
    ...
    */
    ...
    std::cout << "Preparing lane change to lane " << new_lane << std::endl;
    vector<Vehicle> trajectory = {Vehicle(this->lane, this->s, this->v, this->a, this->state)};
...
vector<Vehicle> Vehicle::generate_predictions(int horizon) {
    ...
    */
    ...
    for(int i = 0; i < horizon; i++) {
      double next_s = position_at(i*this->dt);
      double next_v = 0;
      if (i < horizon-1) {
        next_v = (position_at(this->dt*(i+1)) - s)/this->dt;
      }
...
void Vehicle::update_state(string state, int lane, double s, double v, double a) {
    /*
    Sets state and kinematics for ego vehicle using the last state of the trajectory.
    */
    this->state = state;
...
void Vehicle::configure(double target_speed, int num_lanes, double goal_s, int goal_lane, double max_acceleration) {
    /*
